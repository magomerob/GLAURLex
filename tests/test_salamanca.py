...
def test_pdprocesssalamanca_happy_path(tmp_path: Path) -> None:
    ...
    assert set(acciones.columns) == {"user_id", "pos", "token"}

    tokens_601 = acciones[acciones["user_id"] == 601].sort_values("pos")["token"].tolist()
    assert tokens_601 == ["IRALCOLEGIO", "TRABAJAR", "COMER", "BEBER"]

    tokens_602 = acciones[acciones["user_id"] == 602].sort_values("pos")["token"].tolist()
    assert tokens_602 == ["DORMIR", "LEER"]

    tema_02 = pd.read_parquet(tema_02_parquet)
    tokens_603 = tema_02[tema_02["user_id"] == 603].sort_values("pos")["token"].tolist()
